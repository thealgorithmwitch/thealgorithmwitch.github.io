...
async function syncPublicJobsFromRecords(records, options = {}) {
  ...
  let finalJobsJsonCount;
  if (dryRun) {
    finalJobsJsonCount = computedPublicJobsCount;
  } else {
    try {
      const writtenJobs = await readJobs();
      finalJobsJsonCount = Array.isArray(writtenJobs) ? writtenJobs.length : computedPublicJobsCount;
    } catch (readError) {
      logger.error(`[${label}] readback_failed: ${readError.message}`);
      finalJobsJsonCount = computedPublicJobsCount;
    }
  }

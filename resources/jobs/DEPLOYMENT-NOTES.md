...
4. Run `npm run jobs:diagnose-admin-actions`.
5. Run `npm run jobs:apply-admin-actions` only if diagnostics pass.
6. Run `npm run jobs:build-pages` if you need to regenerate pages again after other local edits.

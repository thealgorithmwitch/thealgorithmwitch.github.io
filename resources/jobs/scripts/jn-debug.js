...
const DESCRIPTION_JUNK_PATTERNS = [
  ...
  /\bheaders?\b\s*(?:"\s*)+/i,
  /\b(?:taxonomy|headquarters|employee size|funding)\b/i,
...
const PREFERRED_ROLE_SECTION_HEADINGS = [
  ...
  /position\s+(?:specific\s+)?description/i,
...
function normalizeDescription(description, options = {}) {
  ...
  sentences = dedupeDescriptionSentences(removeBoilerplateSentences(sentences)
    ...
    .filter((sentence) => (
      ...
      || /^(?:Leads?|Delivers?|Executes?|Manages?|Develops?|Supports?|Builds?|Seeks?|Works?|Drives?|Plans|Coordinates?|Optimizes|Partners|Values?|Researches?)\b/i.test(sentence)
    )));
  ...
  const metadataHeavyDescription = /\b(?:career_page|other \d+|ipo \d+|point\s*\(|locality\b|business\/productivity software|business and industrial|cleantech|cleantech & environment|electrical distribution|industrial automation|power generation|oil\s*&\s*gas|headquarters|employee size)\b/i.test(finalDescription);
  ...
  const dominatedByNoise =
    ...
    !( /[a-z]{3,}\s+(?:is|are|will|can|should|must|plans|coordinates|executes|supports|manages|builds|seeks|works|develops|leads|drives|partners|optimizes)/i.test(numericCleaned)
      || /^(?:Leads?|Delivers?|Executes?|Manages?|Develops?|Supports?|Builds?|Seeks?|Works?|Drives?|Plans|Coordinates?|Optimizes|Partners|Values?|Researches?)\b/i.test(numericCleaned) );
